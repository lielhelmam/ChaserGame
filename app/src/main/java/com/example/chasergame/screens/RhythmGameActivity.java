...
public class RhythmGameActivity extends BaseActivity implements GameView.GameEventListener {
    ...
    private com.example.chasergame.views.SpinnerView spinnerView;
    ...
    public void onRequestSpinner(long duration) {
        ...
        spinnerView.start(duration, new com.example.chasergame.views.SpinnerView.SpinnerListener() {
            ...
            public void onFinished(boolean success) {
                gameView.setSpinnerActive(false); // Resume normal gameplay
                if (success) {
                    onNoteHit(1000); // Massive bonus
                    effectOverlay.spawnBurst(spinnerView.getX() + spinnerView.getWidth() / 2f,
                            spinnerView.getY() + spinnerView.getHeight() / 2f,
    ...
    private void updateAccuracy() {
        ...
        tvAccuracy.setText(String.format(Locale.US, "Acc: %.1f%%", acc));
    ...
    private void endGame() {
        ...
        if (!isDead && songData != null && currentUser != null) {
            ...
            if (score > currentPB) {
                if (currentUser.songHighScores == null)
                    currentUser.songHighScores = new java.util.HashMap<>();
                if (currentUser.songRanks == null)
                    currentUser.songRanks = new java.util.HashMap<>();
                if (currentUser.songAccuracies == null)
                    currentUser.songAccuracies = new java.util.HashMap<>();
